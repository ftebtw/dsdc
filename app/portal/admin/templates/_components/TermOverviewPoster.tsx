    ...
    >
      ...
      <div className="relative flex h-full flex-col p-14">
        <div className="flex items-start justify-between text-white">
          ...
          <img src="/images/logos/logo-full.png" alt="DSDC" className="h-14 w-auto object-contain" />
          ...
          {entries.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              {entries.map((entry, idx) => {
                ...
                const instructorNames = entry.instructors
                  .map((i) => i.name.trim())
                  .filter(Boolean)
                  .join(", ");
                return (
                  <div
                    key={entry.id}
                    className="flex items-start gap-6 rounded-2xl border border-warm-300 bg-white px-6 py-4 shadow-sm"
                  >
                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-navy-800 text-xl font-bold text-gold-300">
                      {String(idx + 1).padStart(2, "0")}
                    </div>
                    <div className="flex-1">
                      <div className="text-2xl font-bold text-navy-900">{title}</div>
                      {description ? (
                        <div className="mt-1 text-base text-charcoal/75">{description}</div>
                      ) : null}
                      <div className="mt-2 text-sm text-charcoal/70">
                        {instructorNames ? (
                          <>
                            <span className="font-semibold text-navy-700">{instructorNames}</span>
