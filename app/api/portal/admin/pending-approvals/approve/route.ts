...
export async function POST(request: NextRequest) {
  ...
  const { studentId, classIds } = parsed.data;
  ...
  let updateQuery = admin
    .from("enrollments")
    .update({ status: "active", payment_method: "already_paid" })
    .eq("student_id", studentId)
    .eq("status", "pending_approval");
  if (classIds?.length) {
  ...
  const approvedRows = (updatedRows ?? matchedRows) as Array<{
    id: string;
    class_id: string;
    student_id: string;
  }>;
  const approvedClassIds = [...new Set(approvedRows.map((row) => row.class_id))];
  ...
  if (!studentProfile || !classRowsData || classRowsData.length === 0) {
    return NextResponse.json({ approvedCount: updatedRows?.length ?? matchedRows.length });
  }
  ...
  const coachMap = Object.fromEntries(
    ((coachProfilesResult.data ?? []) as Array<{ id: string; display_name: string | null; email: string }>).map(
      (row) => [row.id, row]
    )
  ) as Record<string, { display_name: string | null; email: string }>;
  const termMap = Object.fromEntries(
    ((termRowsResult.data ?? []) as Array<{ id: string; name: string; start_date: string; end_date: string }>).map(
      (row) => [row.id, row]
    )
  ...
  return NextResponse.json({
    approvedCount: updatedRows?.length ?? matchedRows.length,
  });
}
